...
CREATE TABLE usuari (
    ...
    rol VARCHAR(20) NOT NULL CHECK (rol IN ('administrador', 'personal', 'pacient')),
    id_intern INT UNIQUE,
    id_pacient INT UNIQUE,
    actiu BOOLEAN NOT NULL DEFAULT TRUE,
    data_creacio TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    ultim_acces TIMESTAMP NULL DEFAULT NULL,
    CHECK (
        (rol IN ('administrador', 'personal') AND id_intern IS NOT NULL AND id_pacient IS NULL) OR
        (rol = 'pacient' AND id_pacient IS NOT NULL AND id_intern IS NULL)
    ),
    FOREIGN KEY (id_intern) REFERENCES personal(id_intern),
